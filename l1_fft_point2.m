...
samples = [1,2000];
[x,Fs] = audioread('Click-16-44p1-mono-0.2secs.wav',samples);
sound(x)
figure(1)
stem(x)
...
a0=fft(x); % a0 is fft values
s0=a0;
figure(2)
stem(a0)
axis([0 2000 -1 1]);
title('Fast fourier transform of the recorded signal');
xlabel('Length of the fft spectrum');
ylabel('Amplitude of the fft spectrum');
...
%solve the LP
tic
xp = l1eq_pd(x0, Q, [], y, 1e-2);  %l1eq_pd(randn(N,1), psi(subset,:), [], y_subset, 1e-5);
toc
figure(6)
plot(xp)
stem(xp)
...
% Inverse dicrete cosine transform of reconstructed signal (Ifft)
Xrec = real(ifft(xp));
sound(Xrec,Fs)
% figure(7)
% stem(Xrec)    %restart matlab its a bug
% 
% title('Reconstructed signal at the receiver');
% xlabel('Length of the reconstructed signal using Ifft');
% ylabel('Amplitude of the reconstructed signal using Ifft');
